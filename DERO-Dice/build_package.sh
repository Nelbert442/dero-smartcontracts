...
for PLATFORM in $PLATFORMS; do
  GOOS=${PLATFORM%/*}
  GOARCH=${PLATFORM#*/}
  OUTPUT_DIR="${ABSDIR}/build/derodice_${GOOS}_${GOARCH}"
  BIN_FILENAME="derodice-${GOOS}-${GOARCH}"
  ...
  if [[ "${GOOS}" == "linux" && "${GOARCH}" == "amd64" ]] ; then
    BIN_FILENAME="docker-derodice-${GOOS}-${GOARCH}"
...
if [[ $PLATFORMS_ARM == *"linux"* ]]; then 
  ...
  GOARCH="arm64"
  OUTPUT_DIR="${ABSDIR}/build/derodice_${GOOS}_${GOARCH}"
  CMD="GOOS=linux GOARCH=arm64 go build -gcflags=${GCFLAGS} -o $OUTPUT_DIR/derodice-linux-arm64 $package"
  echo "${CMD}"
  eval $CMD || FAILURES="${FAILURES} ${PLATFORM}"
fi
...
for GOOS in $PLATFORMS_ARM; do
  ...
  for GOARM in 7 6 5; do
    OUTPUT_DIR="${ABSDIR}/build/derodice_${GOOS}_${GOARCH}${GOARM}"
    BIN_FILENAME="derodice-${GOOS}-${GOARCH}${GOARM}"
